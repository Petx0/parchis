...
def test_run_round_creates_expected_files_and_is_internally_consistent(tmp_path):
    ...
    new_state, new_meta, new_history = round_loop.run_round(0, champion_state, meta, [], cfg, run_dir)
...
def test_champion_updates_only_on_forced_promotion(tmp_path, monkeypatch):
    ...
    print("✓ forced promotion: champion state replaced, promoted_history grew, failures reset")
...
def test_champion_unchanged_on_forced_non_promotion(tmp_path, monkeypatch):
    ...
    print("✓ forced non-promotion: champion state byte-identical, promoted_history empty, failures=1")
...
def test_escalation_triggers_after_n_failures_and_resets(tmp_path, monkeypatch):
    ...
    for round_num in range(3):
        champion_state, meta, _history = round_loop.run_round(
